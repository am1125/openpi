...
def train_loop(config: _config.TrainConfig):
    ...
    ####### Adding this section for distillation (Shallow-π) #######
    ...
    - Option 1 (default): set pytorch_weight_path for both teacher
        and initial weigths for distillation (layer subsampled)
    - Option 2 : set pytorch_weight_path_teacher as teacher
        and different initial weigths for distillation.
        For instance, we can keep distilling from previous weights while having the same teacher model.
    """
    ...
    logging.info(f"Loaded teacher pyTorch weights from {teacher_model_path}")
    ...
    if config.pytorch_weight_path is not None:
        ...
        # we assume the n layers will be different in this case.
